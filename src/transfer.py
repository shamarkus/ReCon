...
def get_dataset(dataset):
	if dataset == 'kimore':
		pickle_path = 'data/clean/kimoreDataset.pkl'
...
def main():
	...
	models_directory = os.path.abspath(os.path.join(os.path.dirname(__file__), '../best_models/IR/'))
	...
	train_loader, val_loader = get_dataloader(clean_data, args.exercise, 10)

	criterion = nn.MSELoss()
	optimizer = torch.optim.Adam(newModel.parameters(), lr = 0.0007)
	num_epochs = 2000
